...
export class AnalyticsController {
  ...
  getNuevosClientesHandler = async (req: Request, res: Response) => {
    try {
      const { desde, hasta } = req.query as Record<string, string | undefined>;
      if (!desde || !hasta) {
        return sendError(res, new Error('Debe proporcionar "desde" y "hasta".'), 'Parámetros de fecha inválidos');
      }
      const result = await this.repository.getNuevosClientes(desde, hasta);
      return sendSuccess(res, result);
    } catch (error) {
      return sendError(res, error, 'Error al obtener nuevos clientes');
    }
  };
