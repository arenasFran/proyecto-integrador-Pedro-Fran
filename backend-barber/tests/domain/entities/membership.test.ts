...
describe('Membership entity', () => {
  ...
  describe('create', () => {
    ...
    it('debe crear membresía con paymentMethod local', () => {
      const m = Membership.create({ userId: 'u1', createdBy: 'admin', paymentMethod: 'local', paymentId: 'pay-456' });
      expect(m.paymentMethod).toBe('local');
      expect(m.paymentId).toBe('pay-456');
    });
  });
