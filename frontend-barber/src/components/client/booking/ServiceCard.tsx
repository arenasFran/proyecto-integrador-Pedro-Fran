...
export const ServiceCard: React.FC<ServiceCardProps> = ({ service, isSelected, onSelect }) => {
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.99 }}
      className={`
        relative w-full rounded-[10px] border bg-[#1A1A1A] px-4 py-3 text-left transition-all duration-200
        ${isSelected
    ...
    >
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-[8px] bg-[#FF5C00]/10">
            {renderServiceIcon(service.name, 'w-4 h-4 text-[#FF5C00]')}
          </div>
          <div className="min-w-0">
            <h3 className="text-[13px] font-semibold text-white">{service.name}</h3>
            <p className="text-[11px] text-[#8A8A8A] truncate">{service.description}</p>
          </div>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          <span className="text-[14px] font-bold text-[#FF5C00]">${service.price}</span>
          {isSelected && (
